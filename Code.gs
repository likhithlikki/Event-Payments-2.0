...
// Key: MASTER_DB_SPREADSHEET_ID   Value: <your MASTER_DB spreadsheet ID>

/**
 * Returns the Master DB spreadsheet ID from Script Properties.
 * Throws a descriptive error (not "Invalid argument: id") if not configured.
 */
...
function resolveEventMetadata(params) {
  ...
  const registrySs = SpreadsheetApp.openById(getMasterDbId());
