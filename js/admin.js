 ...
 * Login itself lives on admin-login.html — this file only reads
...
function showDashboard() {
  ...
  if (welcomeEl) welcomeEl.textContent = `${adminSession.name} \u00b7 ${adminSession.role}`;
...
function startSessionClock_() {
  ...
  if (!clockEl || !adminSession.expiresAt) return;
...
function loadSettingsPanel() {
  ...
  panel.querySelectorAll('[data-toggle-key]').forEach(input => {
    input.addEventListener('change', () => {
      if (input.nextElementSibling) input.nextElementSibling.textContent = input.checked ? 'Active' : 'Off';
    });
  });
